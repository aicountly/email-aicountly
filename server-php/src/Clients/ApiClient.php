 ...
 * into Email's database, never refreshed by a cron, never mirrored into a
 * "cache" table, and never embedded into a stored AI summary. A short-lived
 * in-request memo (see $memo) is the only thing that survives a call, and it
 * dies with the process.
 *
 * It is also never handed to the browser raw. A screen that shows a purchase
 * order shows the fields Email asked for, with the source and the time it was
 * read, and re-reads them when the screen is reopened.
 *
...
abstract class ApiClient
{
    ...
    /** Product name this client talks to: manage | purchases | books | calendar | … */
    ...
    {
        return Env::get('APP_PRODUCT_KEY', 'email');
    }
