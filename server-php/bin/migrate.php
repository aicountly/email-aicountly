...
$pdo->exec(
    'CREATE TABLE IF NOT EXISTS email_sql_migrations (
...
$applied = [];
foreach (Db::all('SELECT filename, checksum FROM email_sql_migrations') as $row) {
...
foreach ($files as $path) {
    ...
    try {
        ...
        $pdo->exec($sql);
        $stmt = $pdo->prepare('INSERT INTO email_sql_migrations (filename, checksum) VALUES (:f, :c)');
